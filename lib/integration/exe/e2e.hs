...
getConfig = do
    ...
    alreadyRunningWallet <-
        (readMaybe =<<) <$> lookupEnv "HAL_E2E_ALREADY_RUNNING_WALLET_PORT"

    -- Needed for mithril-client
    setEnvIfMissing
        "GENESIS_VERIFICATION_KEY"
        mithrilPreprodGenesisVerificationKey
    setEnvIfMissing "AGGREGATOR_ENDPOINT" mithrilPreprodAggregatorEndpoint
    setEnvIfMissing
        "ANCILLARY_VERIFICATION_KEY"
        mithrilAncillaryVerificationKey
